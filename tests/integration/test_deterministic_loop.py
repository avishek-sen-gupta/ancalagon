...
def parent(rounds: Rounds, ctx: RunContext) -> Outcome[Rounds]:
    counter = ctx.task_dir / COUNTER
    done = int(ctx.fs.read_text(counter)) if ctx.fs.exists(counter) else 0
    if done == rounds.total:
        return Completed(value=rounds, summary=f"{done} rounds", spent=NOTHING)
...
def test_a_deterministic_parent_is_woken_once_per_child_it_spawns(
    ...
    spec = AgentSpec[rounds_class](
        ...
        role=Role(
            ...
            answer=ClassRef(module="loopkit.rounds", name="Rounds"),
            tools=(),
            budget=Budget(turns=0, tool_calls=0),
        ),
    ...
    assert final["value"] == {"total": ROUNDS}
