...
def test_a_missing_or_unusable_input_exits_two_with_a_message_and_no_traceback(
    ...
    broken_package = broken / "brokenkit"
    broken_package.mkdir()
    (broken_package / "__init__.py").write_text("")
    (broken_package / "unparsable.py").write_text(
        "import pydantic\n\n\nclass Query(pydantic.BaseModel\n    area: str\n"
    )
    refused = failed(
        broken,
        goal_file=str(broken / "goal.md"),
        input_module="brokenkit.unparsable",
        input_class="Query",
    )
    assert "[roles.root] input" in refused.stderr
    assert "brokenkit.unparsable" in refused.stderr
    ...
    unloadable = failed(
        ...
        input_module="nokit.no_such_shapes",
        input_class="Query",
    )
    assert "[roles.root] input" in unloadable.stderr
    assert "nokit.no_such_shapes" in unloadable.stderr

    structured = _case(tmp_path, "structured-without-input")
    shapes_package = structured / "shapekit"
    shapes_package.mkdir()
    (shapes_package / "__init__.py").write_text("")
    (shapes_package / "shapes.py").write_text(QUERY_MODULE)
    incomplete = failed(
        structured,
        goal_file=str(structured / "goal.md"),
        input_module="shapekit.shapes",
        input_class="Query",
    )
