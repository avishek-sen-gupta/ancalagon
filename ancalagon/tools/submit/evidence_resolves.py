...
def _range_fault(cited: Evidence, count: int) -> str:
    if cited.end_line < cited.start_line:
        return f"{cited.path}: end_line {cited.end_line} is before start_line {cited.start_line}"
    if cited.end_line > count:
        return f"{cited.path}: lines {cited.start_line}-{cited.end_line} but the file has {count}"
    return ""
...
def _fault(cited: Evidence, ctx: ToolContext) -> str:
    ...
    return _range_fault(cited, len(lines)) or _mismatch(cited, lines)
