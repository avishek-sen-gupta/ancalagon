...
There is exactly one exemption, granted deliberately and not a precedent: `Citation.other_data` and `CiteArgs.other_data` hold `dict[str, typing.Any]`, because what a run wants kept beside a citation differs per run and naming those fields would fix a shape no run agrees on. It is scoped by both checks that enforce this rule — `check-type-hygiene` exempts a line only in those two files and only when it carries `# type-hygiene: other_data`, and `fp.json` excludes those two files from `python-fp-lint`, which honours no suppression comment. Do not "fix" those fields, and do not read them as licence for a second `Any` anywhere.
